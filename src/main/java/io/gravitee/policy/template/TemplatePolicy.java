...
public class TemplatePolicy implements Policy {
    ...
    private boolean shouldInterrupt(HttpHeaders ctx) {
        return ctx.getAll(TEMPLATE_POLICY_HEADER).stream().anyMatch(header -> header.equalsIgnoreCase(configuration.getErrorKey()));
    }
}
